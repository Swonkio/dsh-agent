/**
 * The dsh-agent regression suite.
 *
...

{
  ...
  const { AgentSurface } = await import(join(ROOT, 'lib/index.js'))
  const { String1D } = await import(join(ROOT, 'lib/field.js'))
  const meter = Object.create(AgentSurface.prototype)
...
{
  const dir = mkdtempSync(join(tmpdir(), 'dsh-agent-test-'))
...
if (process.argv.includes('--pty')) {
  const workspace = mkdtempSync(join(tmpdir(), 'dsh-agent-pty-'))
