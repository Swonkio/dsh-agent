...
# Run after committing; writes dsh-agent-<ref>.tar.gz next to the checkout.
set -euo pipefail
KIT="$(cd "$(dirname "$0")" && pwd)"
REF=$(git -C "$KIT" rev-parse --short HEAD)
OUT="$KIT/../dsh-agent-$REF.tar.gz"
git -C "$KIT" archive --format=tar.gz --prefix="dsh-agent/" -o "$OUT" HEAD
echo "wrote $OUT"
