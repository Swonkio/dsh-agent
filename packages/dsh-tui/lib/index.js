/**
 * dsh-tui — the interactive terminal surface. It mounts over `dsh-base` with
...
export const inject = ['agents', 'sessions', 'agentDefaultModel', 'tools', 'tuiStartup']
...
export class Tui {
  constructor(ctx, startup, exit) {
    ...
    this.editor = new Editor({
      screen: this.screen,
      historyPath: dshHomePath('tui-history'),
  ...
  finishPrint() {
    ...
    if (reason?.kind === 'error') {
      process.stderr.write(`dsh-tui: ${reason.error?.code ?? 'ERROR'}: ${reason.error?.message ?? 'the turn failed'}\n`)
    }
  ...
  setEffort(argument) {
    ...
    if (!Object.hasOwn(Tui.EFFORT, level)) {
      this.warn(`unknown effort "${level}" — use ${Tui.EFFORT_OFFERED.join(', ')}, or default`)
      return
    }
    this.effort = level
    ...
    const marker = Tui.EFFORT[level].replace(/[<>|]|think_/g, '')
  ...
  applyEffort(text) {
    ...
    const marker = Tui.EFFORT[this.effort]
    this.effort = undefined
    return `${marker} ${text}`
  }
 ...
 * for the test suite; the surface reaches it through {@link Tui.complete}. Directory
...
export function apply(ctx) {
  ...
  const tui = new Tui(ctx, startup, exit)
  void tui.run().catch(error => { tui.crash(error) })
}
