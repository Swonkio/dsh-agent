...
```sh
dsh --profile tui                      # open a session in the current directory
dsh --profile tui "fix the flaky test" # open with a first prompt
dsh --profile tui -c                   # continue this directory's last session
dsh --profile tui -r                   # pick a session to resume
dsh --profile tui -p "what is this?"   # one-shot, no UI (like --profile headless)
```
...
|---|---|
| `ctx.cmdlineArgs` | a commander program publishing the `tuiStartup` service |
...
lib/startup.js     the cmdline provider (flags, --help), publishes `tuiStartup`
...
```sh
mkdir -p ~/.dsh/profiles/tui/node_modules
ln -s /path/to/dsh-tui ~/.dsh/profiles/tui/node_modules/dsh-tui
cat > ~/.dsh/profiles/tui/package.json <<'JSON'
{
...
`dsh-tui/node_modules` is a symlink to `$DSH_HOME/profiles/node_modules`, the launcher-maintained flat fallback, so this package resolves `@deepseek-ai/*` and `commander` from the same installation the rest of the tree uses — one cordis instance, no duplicate services.

## This install

- Profile: `~/.dsh/profiles/tui` (bundles `@deepseek-ai/dsh-base` + `dsh-tui`), with `dsh-tui` symlinked into its `node_modules`.
- Launchers: `~/.local/bin/dsh` and `~/.local/bin/dsh-tui` (the latter is `dsh --profile tui`).
...
- Prompt history lives in `~/.dsh/tui-history`.
